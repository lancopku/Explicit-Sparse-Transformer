...
class AdaptiveMask(nn.Module):
    ...
    def __init__(self, max_size, ramp_size, init_val=0, shape=(1,)):
        ...
        self.current_val = nn.Parameter(torch.zeros(*shape) + init_val)  # current_val shape torch.Size([8, 1, 1])
    ...
    def get_current_max_size(self, include_ramp=True):
        current_size = math.ceil(self.current_val.max().item() * self._max_size)  # ValueError: cannot convert float NaN to integer
        if include_ramp:
...
class AdaptiveSpan(nn.Module):
    ...
    def __init__(self, attn_span, adapt_span_loss, adapt_span_ramp,
                 adapt_span_init, adapt_span_cache, nb_heads, div=0, lb=0, **kargs):
        nn.Module.__init__(self)
        self._adapt_cache = adapt_span_cache
        self._max_span = attn_span  # 8192
        ...
        self._mask = AdaptiveMask(max_size=self._max_span,
                                 ...
                                 shape=(nb_heads, 1, 1))
        # for top-k attention selection
        # print('div in adaptive span class', div)
        self.div = div
        self.lb = lb
    ...
    def forward(self, attn, normalize=True):
        ...
        if self.div:
            vk, _ = torch.topk(attn, top_k)
            # print(value)
            tk = vk[:, :, :, -1].unsqueeze(3).expand_as(attn)
            mask_k = torch.lt(attn, tk)
            attn = attn.masked_fill(mask_k, 0).type_as(attn)

        if normalize:
            attn = attn / (attn.sum(-1, keepdim=True) + 1e-8)  # normalize so sum is 1

        attn = attn.view(B, M, -1)
        # 2021 1.27 15:47 刚才还在好奇pos长度是否一致，现在觉得不重要
        # B x M X L_pos
        return attn
