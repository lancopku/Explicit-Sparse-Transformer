...
def compute_attention_component(antecedent,
  ...
  if filter_width == 1:
      # Blas GEMM launch failed : a.shape=(3525, 128), b.shape=(128, 128), m=3525, n=128, k=128
...
def dot_product_sparsemax_attn(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
                          ...
                          save_weights_to=None,
                          dropout_broadcast_dims=None,
                          self_attenion_type=None,
                          selected_topk_attention_type='all',
                                 ):
  """Dot_product_sparse_attention.
  ...
  with tf.variable_scope(
      name, default_name="dot_product_attention", values=[q, k, v]) as scope:
    logits = tf.matmul(q, k, transpose_b=True)  # [..., length_q, length_kv]
    key_len = tf.shape(k)[-2]  # q,k,v[batch, num_heads, length, channels / num_heads]
    query_len = tf.shape(q)[-2]
    num_heads = tf.shape(q)[1]
    bsz = tf.shape(q)[0]
    # 'encoder_self_attention' 'decoder_self_attention' 'decoder_context_attention'
    if bias is not None:
      bias = common_layers.cast_like(bias, logits)
      logits += bias
    # logits [batch, num_heads, q_length, k_length]
    if self_attenion_type == 'decoder_self_attention':
        weights = tf.reshape(sparsemax(tf.reshape(logits, [bsz*num_heads*query_len, key_len]), name="attention_weights"),
                         [bsz, num_heads, query_len, key_len])
    else:
        weights = tf.reshape(sparsemax(tf.reshape(logits, [bsz*num_heads*query_len, key_len]), name="attention_weights"),
                         [bsz, num_heads, query_len, key_len])
    if save_weights_to is not None:
...
def dot_product_entmax15_attn(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  with tf.variable_scope(
      name, default_name="dot_product_entmax15_attn", values=[q, k, v]) as scope:
    logits = tf.matmul(q, k, transpose_b=True)  # [..., length_q, length_kv]
    key_len = tf.shape(k)[-2]  # q,k,v[batch, num_heads, length, channels / num_heads]
    query_len = tf.shape(q)[-2]
    num_heads = tf.shape(q)[1]
    ...
    # logits [batch, num_heads, q_length, k_length]
    if self_attenion_type == 'decoder_self_attention':
        weights = tf.reshape(entmax15(tf.reshape(logits, [bsz*num_heads*query_len, key_len])),
                         [bsz, num_heads, query_len, key_len])
    else:
        weights = tf.reshape(entmax15(tf.reshape(logits, [bsz*num_heads*query_len, key_len])),
                         [bsz, num_heads, query_len, key_len])
...
def dot_product_ensemble_sparse_attention(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  Args:
    ...
    dropout_broadcast_dims: an optional list of integers less than rank of q.
      Specifies in which dimensions to broadcast the dropout decisions.
    d: if d<0 k =d id d> 0 k= key_len/d
    lb: lower bound of k
    before_padding:
    before_softmax:
    softmax_again:
    self_attenion_type: decoder_self_attention decoder_context_attention encoder_self_attention
    selected_topk_attention_type:
  ...
  with tf.variable_scope(
    ...
    key_len = tf.shape(k)[-2]  # q,k,v[batch, num_heads, length, channels / num_heads]
    # 'encoder_self_attention' 'decoder_self_attention' 'decoder_context_attention'
    if selected_topk_attention_type != 'all':
        if selected_topk_attention_type == 'two':
            if self_attenion_type == 'decoder_self_attention':
                d = 0
        elif selected_topk_attention_type != self_attenion_type:
            d = 0
    VERY_NEGATIVE_NUMBER = -1e9
    if d != 0:
        # print('has set', d)
        if d > 0:
            tk = tf.cast(tf.ceil(key_len/d), tf.int32)
            if lb > 0:
                tk = tf.where(tk > lb, tk, lb)
                tk = tf.where(tk > key_len, key_len, tk)
        else:
            tk = -int(d)
            tk = tf.where(tk > key_len, key_len, tk)
    ...
    if bias is not None:
      bias = common_layers.cast_like(bias, logits)
      logits += bias
    # logits [batch, num_heads, length, length]
    if d != 0 and before_softmax and not before_padding:
        num_heads = tf.shape(k)[1]
        query_len = tf.shape(q)[2]
        ek = num_heads * tk
        logits_reshape = tf.reshape(tf.transpose(logits, [0, 2, 1, 3]), [-1, query_len, num_heads*key_len])
        values, _ = tf.nn.top_k(logits_reshape, k=ek, sorted=True)  # [bs,sl,num_heads*k]
        threshold = tf.tile(tf.expand_dims(tf.expand_dims(values[:, :,  -1], -1), 1), [1,  num_heads, 1,  key_len])
        top_k_mask = tf.greater_equal(logits, threshold)  # bs,num_heads, sl,sl]
        logits = tf.add(logits, (1 - tf.cast(top_k_mask, tf.float32)) * VERY_NEGATIVE_NUMBER)
    weights = tf.nn.softmax(logits, name="attention_weights")
    if d != 0 and not before_softmax:
        num_heads = tf.shape(k)[1]
        query_len = tf.shape(q)[2]
        ek = num_heads * tk
        weights_reshape = tf.reshape(tf.transpose(weights, [0, 2, 1, 3]), [-1, query_len, num_heads * key_len])
        values, _ = tf.nn.top_k(weights_reshape, k=ek, sorted=True)  # [bs,sl,num_heads*k]
        threshold = tf.tile(tf.expand_dims(tf.expand_dims(values[:, :, -1], -1), 1), [1, num_heads, 1, key_len])
        top_k_mask = tf.greater_equal(weights, threshold)  # bs,num_heads, sl,sl]
        if softmax_again:
            weights = tf.add(weights, (1 - tf.cast(top_k_mask, tf.float32)) * VERY_NEGATIVE_NUMBER,)
            weights = tf.nn.softmax(weights,)
        else:
            weights = tf.multiply(weights, tf.cast(top_k_mask, tf.float32),)
    if save_weights_to is not None:
      save_weights_to[scope.name] = weights
      save_weights_to[scope.name + "/logits"] = logits
    # Drop out attention links for each head.
...
def dot_product_sparse_attention(q,
                          k,
                          v,
                          bias,
                          dropout_rate=0.0,
  ...
  with tf.variable_scope(
    ...
    if d != 0:
        ...
        if d > 0:
            ...
            tk = -int(d)
            tk = tf.where(tk > key_len, key_len, tk)
    if d != 0 and before_softmax and before_padding:
        values, _ = tf.nn.top_k(logits, k=tk, sorted=True)  # [bs,num_heads,sl,k]
        threshold = tf.tile(tf.expand_dims(values[:, :, :, -1], -1), [1, 1, 1, key_len])
        top_k_mask = tf.greater_equal(logits, threshold)  # bs,num_heads, sl,sl]
        logits = tf.add(logits, (1 - tf.cast(top_k_mask, tf.float32)) * VERY_NEGATIVE_NUMBER)
    if bias is not None:
      bias = common_layers.cast_like(bias, logits)
    ...
    weights = tf.nn.softmax(logits, name="attention_weights")
    if d != 0 and not before_softmax:
        values, _ = tf.nn.top_k(weights, k=tk, sorted=True)  # [bs,num_heads,sl,k]
        threshold = tf.tile(tf.expand_dims(values[:, :, :, -1], -1), [1, 1, 1, key_len])
        top_k_mask = tf.greater_equal(weights, threshold)  # bs,num_heads, sl,sl]
        if softmax_again:
            weights = tf.add(weights, (1 - tf.cast(top_k_mask, tf.float32)) * VERY_NEGATIVE_NUMBER,)
    ...
    # [None, 8, None, None]
    # weights_shape_list= weights.get_shape().as_list()
    # sess = tf.Session()
    # tf.logging.info(weights_shape_list)
    # print("weights \n\n",weights_shape_list)
    # p0 = tf.reduce_mean(tf.reduce_sum(tf.cast(weights == 0, dtype=tf.float32), axis=-1))
    # p3 = tf.reduce_mean(tf.reduce_sum(tf.cast(tf.logical_and(weights > 0, weights < 0.01), dtype=tf.float32), axis=-1))
    # p2 = tf.reduce_mean(tf.reduce_sum(tf.cast(tf.logical_and(weights >= 0.01, weights < 0.1), dtype=tf.float32), axis=-1))
    # p1 = tf.reduce_mean(tf.reduce_sum(tf.cast(tf.logical_and(weights >= 0.1, weights < 1), dtype=tf.float32), axis=-1))
    # p3 = tf.reduce_mean(tf.reduce_sum(tf.multiply(tf.cast(weights > 0, dtype=tf.int32), tf.cast(weights < 0.01, dtype=tf.int32)), axis=-1))
    # p2 = tf.reduce_mean(tf.reduce_sum(tf.multiply(tf.cast(weights >= 0.01, dtype=tf.int32), tf.cast(weights < 0.1, dtype=tf.int32)), axis=-1))
    # p1 = tf.reduce_mean(tf.reduce_sum(tf.multiply(tf.cast(weights >= 0.1, dtype=tf.int32), tf.cast(weights < 1, dtype=tf.int32)), axis=-1))
    # tf.logging.info("p1:%d p2:%d p3:%d p0:%d" % (p1, p2, p3, p0))
    # tf.Print(p1, [p1], message="number of p1 is: ")
    # tf.print(p1)
    # tf.logging.info("p1:%d p2:%d p3:%d" % (p1.eval(), p2.eval(), p3.eval()))
    return tf.matmul(weights, v)
...
def multihead_attention(query_antecedent,
  ...
  with tf.variable_scope(name, default_name="multihead_attention",
    ...
    elif attention_type == 'sparse_dot_product':
        x = dot_product_sparse_attention(q, k, v, bias, dropout_rate, image_shapes,
                                         save_weights_to=save_weights_to,
                                         make_image_summary=make_image_summary,
                                         dropout_broadcast_dims=dropout_broadcast_dims,
                                         d=hparams.d, lb=hparams.lb, before_padding=hparams.before_padding,
                                         before_softmax=hparams.before_softmax, softmax_again=hparams.softmax_again,
                                         self_attenion_type=self_attenion_type,
                                         selected_topk_attention_type=hparams.selected_topk_attention_type)
    elif attention_type == 'ensemble_sparse_attn':
        x = dot_product_ensemble_sparse_attention(q, k, v, bias, dropout_rate, image_shapes,
                                         save_weights_to=save_weights_to,
                                         make_image_summary=make_image_summary,
                                         dropout_broadcast_dims=dropout_broadcast_dims,
                                         d=hparams.d, lb=hparams.lb, before_padding=hparams.before_padding,
                                         ...
                                         self_attenion_type=self_attenion_type,
                                         selected_topk_attention_type=hparams.selected_topk_attention_type)
    elif attention_type == 'sparsemax_attn':
        x = dot_product_sparsemax_attn(q, k, v, bias, dropout_rate, image_shapes,
                                                  save_weights_to=save_weights_to,
                                                  make_image_summary=make_image_summary,
                                                  dropout_broadcast_dims=dropout_broadcast_dims,
                                                  self_attenion_type=self_attenion_type,
    ...
    elif attention_type == 'entmax15_attn':
        x = dot_product_entmax15_attn(q, k, v, bias, dropout_rate, image_shapes,
                                                  save_weights_to=save_weights_to,
                                                  make_image_summary=make_image_summary,
                                                  dropout_broadcast_dims=dropout_broadcast_dims,
                                                  self_attenion_type=self_attenion_type,
