...
def jingjing_norm(inputs):
  max_value = tf.max(inputs, -1, True)[0]  # .repeat(1,1,self.d_model) # detach()
  min_value = tf.min(inputs, -1, True)[0]  # .repeat(1,1,self.d_model) # detach()
  new_min = tf.ones(max_value.size(), device='cuda').type_as(inputs) * -1
  input_norm = (inputs - min_value) / (max_value - min_value) * 2 + new_min
  # print(index)
  # print(pow(1+self.ada,index))
  input_norm = input_norm  # self.w * torch.tanh(input_norm) * pow(1+self.ada,index))+self.b
  return input_norm
